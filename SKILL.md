...
defaults give you the built-in base image (built locally on first `akf up`), `node` user, workspace
at `/workspaces/<basename>`, and `claude --dangerously-skip-permissions` as the command.
